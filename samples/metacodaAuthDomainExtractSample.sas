...
Sample: metacodaAuthDomainExtractSample.sas

Purpose:
   Demonstrates the use of the metacodaAuthDomainExtract macro.
...
%include '../sasautos/metacodaauthdomainextract.sas';
...
* Sample 1: extract basic attributes for all AuthenticationDomain objects;

%metacodaAuthDomainExtract(table=work.authDomains)

* -----------------------------------------------------------------------------;

* Sample 2: extract basic attributes for all AuthenticationDomain objects
  as above but also generate debug info and capture the XML files in /tmp
  ;

%metacodaAuthDomainExtract(
    table=work.authDomains,
    xmlDir=/tmp,
    debug=1
    )

title1 "AuthenticationDomain objects";

proc contents data=work.authDomains;
run;

proc print data=work.authDomains label width=min;
run;
